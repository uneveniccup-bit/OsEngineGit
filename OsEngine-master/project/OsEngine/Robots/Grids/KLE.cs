...
namespace OsEngine.Robots.Grids
{
    ...
    public class KLE : BotPanel
    {
        ...
        private decimal lastPriceForClose;
        ...
        public KLE(string name, StartProgram startProgram) : base(name, startProgram)
        {
        ...
        private void LogicCreateGrid(List<Candle> candles)
        {
            ...
            decimal lastMiddleLine = _bollinger.DataSeries[2].Last;
        ...
        private void LogicCloseGrid(List<Candle> candles)
        {
            ...
            {
                lastPriceForClose = _tab.PriceBestAsk;
            }
            else if (gridSide == Side.Sell)
            {
                lastPriceForClose = _tab.PriceBestBid;
            }
            ...
            if (gridSide == Side.Buy
                && lastPriceForClose > lastMiddleLine)
            {
                grid.Regime = TradeGridRegime.Off;
                if (grid.HaveOpenPositionsByGrid == true
                && grid.Regime == TradeGridRegime.Off)
                    grid.Tab.CloseAllOrderInSystem();
                for (int i = 0; i < positions.Count; i++)
                {
                    Position pos = positions[i];
                    grid.Tab.CloseAtLimit(pos, lastPriceForClose, positions.Count);
                }
            }
            else if (gridSide == Side.Sell
                && lastPriceForClose < lastMiddleLine)
